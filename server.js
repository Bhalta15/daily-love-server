...
app.post("/notificar", async (req, res) => {
  const { oneSignalId, tipo, nombreUsuario, preview } = req.body;

  if (!oneSignalId || !tipo) {
    return res.status(400).json({ error: "Faltan datos" });
  }

  // Título base por tipo
  const tituloBase = {
  ...
  // Cuerpo: si hay preview lo muestra, si no solo el título base
  const titulo   = "Daily Love 💕";
  const cuerpo   = preview
    ? `${tituloBase[tipo] || "Tu pareja te dejó algo ❤️"}\n"${preview}"`
    : (tituloBase[tipo] || "Tu pareja te dejó algo ❤️");
